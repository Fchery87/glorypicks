...
class AIEnhancer:
    ...
    def enhance_signal(
        ...
        regime_alignment = self._calculate_regime_alignment(pattern_type, regime, base_strength)
    ...
    def _calculate_regime_alignment(
        self, pattern_type: str, regime: MarketRegime, signal_strength: float
    ) -> float:
        """
    ...
    def _get_pattern_performance(
        self, pattern_type: str, symbol: str, timeframe: str
    ...
    def _calculate_false_signal_risk(
        self, df: pd.DataFrame, pattern_type: str, regime: MarketRegime
    ) -> float:
        """
    ...
    def _calculate_confluence_bonus(
        self, ict_signals: list[Any] | None, smc_signals: list[Any] | None
    ) -> float:
        """
    ...
    def _generate_ai_rationale(
        ...
        regime: MarketRegime,
        regime_alignment: float,
        pattern_perf: PatternPerformance | None,
        false_signal_risk: float,
        confluence_bonus: float,
    ) -> list[str]:
        ...
        if regime_alignment >= 75:
            ...
            rationale.append(
                f"Weak regime alignment ({regime_alignment:.0f}%) - counter-trend risk"
            )

        # Historical performance
        if pattern_perf and pattern_perf.total_signals > 10:
            rationale.append(
                f"Historical win rate: {pattern_perf.win_rate:.1f}% ({pattern_perf.total_signals} samples)"
            )
