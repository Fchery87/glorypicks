        ...
        for i in range(len(highs)):
            for j in range(i + 1, len(highs)):
                if abs(highs[i] - highs[j]) / highs[i] < 0.001:  # 0.1% tolerance
                    ...
                    # Avoid duplicates
                    if not any(
                        abs(p.price_level - highs[i]) / highs[i] < 0.001
        ...
        for i in range(len(lows)):
            for j in range(i + 1, len(lows)):
                if abs(lows[i] - lows[j]) / lows[i] < 0.001:  # 0.1% tolerance
                    ...
                    if not any(
                        abs(p.price_level - lows[i]) / lows[i] < 0.001
                        for p in self.liquidity_pools
                        if p.type == "sell_side"
                    ):
                        self.liquidity_pools.append(pool)
    ...
    def _detect_liquidity_sweeps(
        ...
        for pool in self.liquidity_pools:
            ...
            if pool.type == "buy_side":
                ...
                high_violation = any(
                    candle > pool.price_level * 1.001 for candle in recent_data["high"]
                )
                ...
                low_violation = any(
                    candle < pool.price_level * 0.999 for candle in recent_data["low"]
                )
                rejection = current_price > pool.price_level
