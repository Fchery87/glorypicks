...
class ICTPhase1Enhancements:
    ...
    def calculate_pd_arrays(self, candles: list[Candle], lookback: int = 50) -> PDArrayInfo:
        """
    ...
    def detect_liquidity_sweeps(
        ...
        for i, candle in enumerate(recent_candles):
            ...
            for _pool_name, pool_level in liquidity_pools.get("buy_side", {}).items():
                ...
                if candle.h >= pool_level * 1.001:  # 0.1% threshold
                    ...
                    is_bearish_reversal = (
                        candle.c < candle.o  # Bearish candle
                        or (i > 0 and candle.c < recent_candles[i - 1].c)  # Lower close
                    )
            ...
            for _pool_name, pool_level in liquidity_pools.get("sell_side", {}).items():
                # Price touched or slightly exceeded the pool
                if candle.l <= pool_level * 0.999:  # 0.1% threshold
                    # Check for reversal (close above open or above previous close)
                    is_bullish_reversal = (
                        candle.c > candle.o  # Bullish candle
                        or (i > 0 and candle.c > recent_candles[i - 1].c)  # Higher close
                    )
