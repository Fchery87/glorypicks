...
class KillZoneInfo:
    ...
    is_active: bool
    time_until_next: int | None  # Minutes until next kill zone
    time_remaining: int | None  # Minutes remaining in current kill zone
    session: SessionType
...
class KillZoneDetector:
    ...
    def get_current_kill_zone(self, timestamp: int | None = None) -> KillZoneInfo:
        """
    ...
    def _create_kill_zone_info(
        self, zone_type: KillZoneType, is_active: bool, current_time: time
    ) -> KillZoneInfo:
    ...
    def analyze_candles_in_kill_zones(
        self, candles: list[Candle]
    ) -> dict[KillZoneType, list[Candle]]:
        """
    ...
    def get_kill_zone_bias(self, zone_type: KillZoneType) -> dict:
        ...
        biases = {
            ...
            KillZoneType.LONDON_CLOSE: {
                ...
                "optimal_strategies": ["Mitigation zones", "Order block retests", "Range trading"],
            },
            ...
            KillZoneType.OFF_HOURS: {
                ...
                "optimal_strategies": ["None - wait for kill zone"],
            },
        }
    ...
    def should_trade_signal(
        self, signal_timestamp: int, signal_strength: float
    ) -> tuple[bool, str]:
        """
        ...
        if info.is_active and info.optimal_for_entries and signal_strength >= 50:
            return True, "Signal in optimal kill zone with moderate strength"
        ...
        if info.zone_type == KillZoneType.OFF_HOURS:
            ...
            return (
                False,
                f"{info.time_until_next} minutes until next kill zone - wait for better timing",
            )
