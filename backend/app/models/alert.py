...
class AlertCreate(BaseModel):
    ...
    alert_type: AlertType = Field(..., description="Type of alert condition to monitor")

    # Condition parameters (based on alert_type)
    strength_threshold: float | None = Field(
        None, ge=0, le=100, description="Strength percentage threshold (0-100)"
    )

    price_threshold: float | None = Field(
        None, gt=0, description="Price threshold for price alerts"
    )

    # Optional settings
    enabled: bool = Field(True, description="Whether alert is initially enabled")

    # Notification settings
    send_notification: bool = Field(True, description="Send browser notification when triggered")

    play_sound: bool = Field(True, description="Play sound when triggered")

    sound_name: Literal["default", "chime", "bell", "alert"] = Field(
        "default", description="Sound to play when triggered"
    )

    # Optional expiration
    expires_at: datetime | None = Field(None, description="Optional expiration time for the alert")

    notes: str | None = Field(None, max_length=500, description="Optional notes for this alert")
    ...
    def validate_strength_threshold(cls, v: float | None, info) -> float | None:
        ...
        if info.data.get("alert_type") in ["strength_above", "strength_below"]:
            if v is None:
                raise ValueError("strength_threshold is required for strength_above/below alerts")
        return v
    ...
    def validate_price_threshold(cls, v: float | None, info) -> float | None:
        ...
        if info.data.get("alert_type") in ["price_above", "price_below"]:
            if v is None:
                raise ValueError("price_threshold is required for price_above/below alerts")
        return v
...
class AlertUpdate(BaseModel):
    ...
    sound_name: Literal["default", "chime", "bell", "alert"] | None = None
